...
class Policy(nn.Module):
    def __init__(self, num_actions, frame_dim):
        ...
        conv_width = conv2d_size_out(conv2d_size_out(conv2d_size_out(frame_dim[1], kernel_size=FIRST_LAYER_KERNEL_SIZE,
                                                                     stride=FIRST_LAYER_STRIDE),
                                                     kernel_size=SECOND_LAYER_KERNEL_SIZE, stride=SECOND_LAYER_STRIDE),
                                     kernel_size=THIRD_LAYER_KERNEL_SIZE, stride=THIRD_LAYER_STRIDE)
        conv_height = conv2d_size_out(conv2d_size_out(conv2d_size_out(frame_dim[0], kernel_size=FIRST_LAYER_KERNEL_SIZE,
                                                                      stride=FIRST_LAYER_STRIDE),
                                                      kernel_size=SECOND_LAYER_KERNEL_SIZE, stride=SECOND_LAYER_STRIDE),
                                      kernel_size=THIRD_LAYER_KERNEL_SIZE, stride=THIRD_LAYER_STRIDE)
